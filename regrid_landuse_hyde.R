...
regrid_landuse_hyde <- function( year, idx, grid.out, datadir, verbose=FALSE ){ 
  ...
  ## Write NetCDF output
  system( paste( "mkdir -p ", datadir, "/annual", sep="" ) )
  cdf.write(
            out.regrid.landuse$lu.rel[,,1], "crop",
            out.regrid.landuse$lon,
            out.regrid.landuse$lat,
            paste( datadir, "/annual/landuse_hyde32_gcp2017_", grid.out, "_i", idx, "_", year, ".cdf", sep="" ),
...
# fyrs <- read.table( paste( datadir, "/fyrlist.txt", sep="" ), col.names=c("year") )$year

idx <- 0
for (yr in sort(yrs)){
  idx <- idx + 1
  regrid_landuse_hyde( as.character(yr), sprintf("%03i", idx), "halfdeg", datadir, verbose=TRUE )
  regrid_landuse_hyde( as.character(yr), sprintf("%03i", idx), "1x1deg",  datadir, verbose=TRUE )
}
